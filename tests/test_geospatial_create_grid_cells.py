...
def test_geospatial_create_polygons(test_data, tmpdir):
    ...
    cells = create_polygons(field_corners=test_data.plot_bounds, plot_geojson_path=test_data.plot_points, out_path=filename,
...
def test_geospatial_create_polygons_single_points(test_data, tmpdir):
    ...
    cache_dir = tmpdir.mkdir("cache")
    filename = os.path.join(cache_dir, 'test_out.geojson')
    cells = create_polygons(field_corners=test_data.point_crop, plot_geojson_path=test_data.plot_points, out_path=filename,
                            num_rows=8, range_length=4, column_length=0.5)
    assert len(cells) == 32 
    assert os.path.exists(filename)
...
def test_geospatial_create_grid_cells(test_data, tmpdir):
    ...
    cache_dir = tmpdir.mkdir("cache")
    filename = os.path.join(cache_dir, 'test_out.geojson')
    cells = create_grid_cells(field_corners==test_data.point_crop, out_path=filename, alley_size=0,
