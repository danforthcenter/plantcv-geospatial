...
from plantcv.geospatial.create_shapes import flexible
...
def test_geospatial_shape_flexible(debug, test_data, tmpdir):
    ...
    # Debug mode
    params.debug = debug
    _ = flexible(img=img, field_corners_path=test_data.plot_bounds, plot_geojson_path=test_data.plot_points,
...
def test_geospatial_shape_flexible_single_points(test_data, tmpdir):
    ...
    # Read in test data
    img = joblib.load(test_data.rgb_pickled)
    _ = flexible(img=img, field_corners_path=test_data.point_crop, plot_geojson_path=test_data.plot_points,
                 out_path=filename, num_rows=8, range_length=4, row_length=0.5)
    assert os.path.exists(filename)
